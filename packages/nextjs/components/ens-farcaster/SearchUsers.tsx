    ...
    return (
        <div className="relative w-full" ref={wrapperRef}>
            ...
            {isOpen && (search.length > 0) && (
                <div className="absolute top-full left-0 right-0 mt-3 bg-base-100 border border-base-content/10 rounded-[1.5rem] shadow-2xl z-[100] overflow-hidden backdrop-blur-xl">
                    <div className="p-2 flex flex-col gap-1">
                        {/* Direct Match / Fallback */}
                        {(() => {
                            const exactName = search.toLowerCase().endsWith(".eth")
                                ? search.toLowerCase()
                                : `${search.toLowerCase()}.eth`;

                            if (!suggestions.includes(exactName) && search.length > 2) {
                                return (
                                    <button
                                        onClick={() => handleNavigate(search)}
                                        className="w-full flex items-center gap-3 p-3 hover:bg-primary/10 rounded-xl transition-colors text-left group"
                                    >
                                        <div className="w-10 h-10 rounded-full overflow-hidden border border-base-content/10 shrink-0">
                                            <EnsAvatar name={exactName} />
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="text-sm font-black group-hover:text-primary transition-colors">Go to "{exactName}"</span>
                                            <span className="text-[10px] opacity-40 uppercase font-mono italic">Direct Access</span>
                                        </div>
                                    </button>
                                );
                            }
                        ...
                        {/* Suggestions divider if needed */}
                        {suggestions.length > 0 && search.length > 2 && !suggestions.includes(
                            search.toLowerCase().endsWith(".eth") ? search.toLowerCase() : `${search.toLowerCase()}.eth`
                        ) && (
                                <div className="h-px bg-base-content/5 mx-3 my-1" />
                            )}
